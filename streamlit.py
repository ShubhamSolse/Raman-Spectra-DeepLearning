...
def run(model_name):
    if args.c == 2:
        v = pd.read_csv('bin.csv').values
    else:
        v = pd.read_csv('multi.csv').values
    x = v[:, :args.d]
    y = v[:, -1]
    ...
    def classifier(tid, vid, tag=1):
        ...
        if args.c == 2:
            ...
            cm = confusion_matrix(yv, predict)  # Compute confusion matrix
            res = [
    ...
    st.write(df.std())
# Title
st.title("Deep Learning models on Raman Spectra")

# Dropdown to select classes
classes = {"CNN": CNNNet, "CLR": CLR, "LSTM":LSTMNet, "Transformer": Transformer}  # Replace with actual mapping
selected_class_name = st.selectbox("Select a class to execute:", list(classes.keys()))

# Button to run the selected class
if st.button("Run"):
    selected_class = classes[selected_class_name]()
    run(selected_class_name)
